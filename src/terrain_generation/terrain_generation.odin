...
init_terrain_instances :: proc() {
	...
	append(
		&shared.Terrain_instances,
		shared.TerrainInstance {
			model_type = .LibertyIsland,
			model = terrain_models.terrain.liberty_island.model,
			position = rl.Vector3{0.0, 0.0, -10.0},
			scale = 1.0,
			bounds = terrain_models.terrain.liberty_island.bounding_box,
		},
	)
